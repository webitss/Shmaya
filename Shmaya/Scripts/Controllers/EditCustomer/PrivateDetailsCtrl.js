	...
	function ($scope, $rootScope, $timeout, connect, $filter, $location, codeTablesName, tablesId, alerts, codeTablesId, createDialog) {
		...
		$scope.saveDetails = function () {
			if ($scope.user.dtCreateDate && $scope.user.dtResetCommunication) {
				//בדיקת תקינות לתאריכים להבטיח שיתבצעו על פי סדר כרונולוגי נכון
			...
			if ($scope.isEdit == true) {
				...
				connect.post(true, 'UserUpdate', { user: $scope.user, iUserManagerId: $rootScope.user.iUserId, changeEligibility: $scope.changeEligibility }, function (result) {
					if (result && result > 0) {
